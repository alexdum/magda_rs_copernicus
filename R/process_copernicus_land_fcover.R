...
files1 <- 
  list.files("/data/MTDA/BIOPAR/BioPar_FCOVER300_V1_Global/", recursive = T, full.names = T, pattern = ".nc$") %>%
...
for (i in 1:length(files)) {
  ...
  for (e in 1:length(exts)) {
    ...
    file_name <- paste0("FCOVER_", format(day, "%Y%m%dXX"))
    file_path <- paste0("grids/FCOVER/",names(exts[e]),"/", format(day, "%Y"))
...
r <- rast(list.files("grids/FCOVER/fr/", full.names = T, pattern = ".tif$", recursive = T))
